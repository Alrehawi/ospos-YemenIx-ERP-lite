...
use App\Libraries\CustomerService;
...
{
    $first_name = $this->nameize($this->request->getPost('first_name'));
    $last_name  = $this->nameize($this->request->getPost('last_name'));
    ...
    $customer_data = [
        ...
        'discount_type'     => $this->request->getPost('discount_type') ?? PERCENT,
        'package_id'        => $this->request->getPost('package_id') ?: null,
        'taxable'           => $this->request->getPost('taxable') != null,
        'date'              => $date_formatter->format('Y-m-d H:i:s'),
        'employee_id'       => $this->request->getPost('employee_id', FILTER_SANITIZE_NUMBER_INT),
        'sales_tax_code_id' => $this->request->getPost('sales_tax_code_id') ?: null
    ];

    // 🔥 Prevent duplicate ID on insert
    ...
    $customer_id_final = $this->customer->save_customer($person_data, $customer_data, $customer_id);
    ...
    // ------------------------
    // Get Final ID (CI4 Safe)
    // ------------------------
    $customer_id_final = ($customer_id == NEW_ENTRY)
        ? $this->customer->db->insertID()
        : $customer_id;

    // ------------------------
    // Accounting Hook
    // ------------------------
    $customer_id_final = $this->customer->get_info_by_email($email)->person_id ?? 0;
    $this->customerService->after_save($customer_id_final);
    ...
    echo json_encode([
        'success' => true,
        'message' => ($customer_id == NEW_ENTRY
            ? lang('Customers.successful_adding')
            : lang('Customers.successful_updating'))
            . " $first_name $last_name",
        'id' => $customer_id_final
    ]);
}
    /**
