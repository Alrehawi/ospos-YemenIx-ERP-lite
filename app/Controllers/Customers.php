...
class Customers extends Persons
{
    ...
    public function postSave(int $customer_id = NEW_ENTRY): void
    {
        ...
        if ($this->customer->save_customer($person_data, $customer_data, $customer_id)) {
                            ...
                            if ($customer_id == NEW_ENTRY && !empty($customer_data['account_number']))
                {
                    $customer_id_final = $this->customer->db->insertID();

                    $this->create_customer_account($customer_id_final, $customer_data['account_number'], $person_data);
                }
    ...
    private function create_customer_account($customer_id, $account_code, $person_data)
{
    // اسم الحساب
    $account_name = trim(($person_data['first_name'] ?? '') . ' ' . ($person_data['last_name'] ?? ''));
    $db = \Config\Database::connect();
    if (empty($account_name)) {
        $account_name = 'Customer ' . $account_code;
    }

    // تحقق إذا الحساب موجود
    $existing = $db->table('ospos_accounts')
        ->where('account_code', $account_code)
        ->get()
        ->getRow();

    if ($existing) {
        return; // لا تكرر الحساب
    }

    // parent (Accounts Receivable)
    $parent = $db->table('ospos_accounts')
    ...
    // إنشاء الحساب
    $db->table('ospos_accounts')->insert([
