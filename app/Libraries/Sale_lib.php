...
// 🔥 PAYMENT ENGINE
// =======================
public function recalculate_payments(): void
{
    $payments = $this->get_payments();

    // لا يوجد payments → لا تفعل شيء
    if (empty($payments)) {
        return;
    }
    ...
    foreach ($payments as $key => $payment)
    {
        ...
        $new_payments[$key] = $payment;
        $paid_total = bcadd($paid_total, $payment['payment_amount']);
    }

    // ❗ لا نضيف Due هنا نهائيًا

    $this->set_payments($new_payments);
}
}
