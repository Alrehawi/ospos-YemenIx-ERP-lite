...
class Item_kit extends Model
{
    ...
    public function get_kit_cost(int $item_kit_id): float
{
    // 🧾 بيانات الكيت
    $kit = $this->get_info($item_kit_id);

    $builder = $this->db->table('item_kit_items');
    $builder->select('item_kit_items.quantity, items.cost_price');
    $builder->join('items', 'items.item_id = item_kit_items.item_id');
    $builder->where('item_kit_items.item_kit_id', $item_kit_id);
    ...
    // 🎯 تطبيق الخصم إن وجد
    if (!empty($kit->kit_discount))
    {
        if ($kit->kit_discount_type == 0) // percent
        {
            $total_cost -= ($total_cost * ($kit->kit_discount / 100));
        }
        else // fixed
        {
            $total_cost -= $kit->kit_discount;
        }
    }

    return max($total_cost, 0); // 🔒 لا تسمح بقيمة سالبة
}
